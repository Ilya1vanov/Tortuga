...
public class ProducerTest {
    ...
    private MultipleFactory factory = mock(MultipleFactory.class);
    ...
    public void producerShouldThrowIAEForInvalidInput(String name, Integer num) throws Exception {
        ...
        order = new ProductionOrder<>(client, SUT, notations);
        // act
        SUT.produce(order);
    }
    ...
    public void producerShouldProduce(String name, Integer num) throws Exception {
        // arrange
        notations.add(new Pair<>(name, num));
        when(factory.create(name, num)).thenReturn(new ArrayList<>(Collections.nCopies(num, producible)));
        order = new ProductionOrder<>(client, SUT, notations);
        ...
        // assert
        verify(factory).create(name, num);
    ...
    public void producerShouldProduceRepeatedOrders(String[] names, Integer[] nums) throws Exception {
        factory = mock(MultipleFactory.class, withSettings().useConstructor().defaultAnswer(CALLS_REAL_METHODS));
        SUT = new Producer<>(factory);
        // arrange
        int totalNum = 0;
